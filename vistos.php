...
$sql = "SELECT (Totales-Capitulos) as total FROM `emision` WHERE Dia='" . $nombre_dia . "'and Emision='Emision' ORDER BY `total` ASC limit 1";
...
if ($resta <= 0) {
    ...
    Swal.fire({
        ...
        window.location = "./emision.php?enviar=&accion=HOY.php";
    });
    </script>';
} else {
    echo "Mayor a Cero<br>";
    $consulta = "UPDATE emision SET Capitulos = Capitulos + 1 WHERE emision.dia LIKE '%" . $nombre_dia . "%' AND Emision='Emision' AND ID_Emision>1;";
