...
if (isset($_POST['mix'])) {
    ...
    $sqlInsert = "INSERT INTO `mix_ed`(`ID`) VALUES ('')";
    ...
    try {
        ...
        if ($stmt->execute()) {
            ...
            $alertTitle = 'Ending Eliminado!';
            $alertText = 'Eliminando ED ' . htmlspecialchars($ed) . ' de ' . htmlspecialchars($nombre) . '';
            $alertType = 'success';
            $redireccion = htmlspecialchars($link);
            ...
            $alertText = 'No se pudo eliminar el Ending. Por favor, inténtelo de nuevo.';
            $alertType = 'error';
            $redireccion = htmlspecialchars($link);
