...
<body>
    ...
    $consulta = "SELECT * FROM `ed` where ID='$id' ORDER BY `ID` DESC";
    ...
    if ($resultados->num_rows > 0) {
        ...
        while ($row = $resultados->fetch_assoc()) {
            ...
            $sql1 = "SELECT autor.Autor, ed.ID, ((SELECT COUNT(*) FROM op WHERE op.ID_Autor = autor.ID) + (SELECT COUNT(*) FROM ed WHERE ed.ID_Autor = autor.ID)) AS Repeticiones FROM autor JOIN ed ON autor.ID = ed.ID_Autor WHERE ed.ID = '$id' AND autor.Autor != '' HAVING Repeticiones > 3;";
            ...
            $sql2 = "SELECT ed.*, anime.Anime FROM `ed` INNER JOIN anime ON ed.ID_Anime = anime.id WHERE ed.ID = '$id'";
